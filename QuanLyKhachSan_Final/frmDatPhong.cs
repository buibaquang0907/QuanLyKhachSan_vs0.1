...
namespace QuanLyKhachSan_Final
{
    public partial class frmDatPhong : Form
    {
        ...
        {
            List<PHIEUTHUEPHONG> listPhieuDat = context.PHIEUTHUEPHONGs.ToList();
            ...
            List<PHONG> listPhong = context.PHONGs.Where(p => p.LOAIPHONG.tenLoai == cmbLoaiPhong.Text).ToList();
            ...
            try
            {
                if (txtMaPhieuThue.Text != "" && txtMaKH.Text != "" && txtTenKH.Text != "" && txtSDT.Text != "" && txtDiaChi.Text != "" && txtCCCD.Text != "")
                {
                    ...
                    else if (kiemTraMaKH(txtMaKH.Text) == true)
                    {
                        ...
                        context.PHIEUTHUEPHONGs.Add(newPhieuThemPhong);
                        context.SaveChanges();
        ...
        {
            PHONG i = (PHONG)context.PHONGs.FirstOrDefault(p => p.tenPhong == cmbPhong.Text);
            return i.maPhong;
        }

        private bool kiemTraSoNguoiO(string soNguoi)
        {
            PHONG phongChon = context.PHONGs.FirstOrDefault(p => p.tenPhong == cmbPhong.Text);
        ...
        private void btnXoaPTP_Click(object sender, EventArgs e)
        {
            ...
            {   
                ...
                {
                    PHIEUTHUEPHONG dbDelete = context.PHIEUTHUEPHONGs.FirstOrDefault(p => p.maPhieuThue == txtMaPhieuThue.Text);
                    HOADON maPhieuinHoaDon = context.HOADONs.FirstOrDefault(p => p.maPhieuThue == txtMaPhieuThue.Text);
                    if (dbDelete != null)
                    {
                        ...
                        else
                        {
                            try
                            {
                                ...
                                {
                                    context.PHIEUTHUEPHONGs.Remove(dbDelete);
                                    context.SaveChanges();
                                    List<PHIEUTHUEPHONG> listPhieuThue = context.PHIEUTHUEPHONGs.ToList();                                   
