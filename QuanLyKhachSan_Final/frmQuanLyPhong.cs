...
namespace QuanLyKhachSan_Final
{
    public partial class frmQuanLyPhong : Form
    {
        ...
        {
            List<LOAIPHONG> listLoaiPhong = context.LOAIPHONGs.ToList();
        ...
        private void btnThemLoai_Click(object sender, EventArgs e)
        {
            try
            {
                if(txtMaLoai.Text != "" && txtTenLoai.Text != "" && txtGiaThue.Text != "")
                {
                    ...
                    {
                        LOAIPHONG loaiPhong = context.LOAIPHONGs.FirstOrDefault(p => p.maLoai == txtMaLoai.Text);
                        ...
                        context.SaveChanges();
                        List<LOAIPHONG> listLoai = context.LOAIPHONGs.ToList();
        ...
        {
            LOAIPHONG loaiPhongDelete = context.LOAIPHONGs.FirstOrDefault(p => p.maLoai == txtMaLoai.Text);


            if (loaiPhongDelete != null)
            {
                PHONG phongCoMaPhong = context.PHONGs.FirstOrDefault(p => p.maLoai == txtMaLoai.Text);
                ...
                else
                {
                    try
                    {
                        ...
                        {
                            context.LOAIPHONGs.Remove(loaiPhongDelete);
                            context.SaveChanges();
                            List<LOAIPHONG> listloaiPhong = context.LOAIPHONGs.ToList();
                            BrindGridLoaiPhong(listloaiPhong);
        ...
        private void btnThemPhong_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtMaPhong.Text != "" && txtTenPhong.Text != "" && txtSoKhachToiDa.Text != "")
                {
                    ...
                    {
                        PHONG Phong = context.PHONGs.FirstOrDefault(p => p.maPhong == txtMaPhong.Text);
        ...
        {
            PHONG PhongDelete = context.PHONGs.FirstOrDefault(p => p.maPhong == txtMaPhong.Text);

            if (PhongDelete != null)
            {
                PHIEUTHUEPHONG phieuThue = context.PHIEUTHUEPHONGs.FirstOrDefault(p => p.maPhong == txtMaPhong.Text);
                if(phieuThue == null) 
                {
                    try
                    {
                        ...
                        if (re == DialogResult.Yes)
                        {
                            context.PHONGs.Remove(PhongDelete);
                            context.SaveChanges();
                            List<PHONG> listPhong = context.PHONGs.ToList();
                            BrindGridPhong(listPhong);
                            MessageBox.Show("Xóa thành công!", "THÔNG BÁO", MessageBoxButtons.OK, MessageBoxIcon.Information);
